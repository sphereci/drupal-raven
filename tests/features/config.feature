...
Feature: Config
  ...
  Scenario: See config page when user 1
    ...
    And the "raven_service" variable is set to "demo"
    And I am on "/admin/config"
    Then I should see a "h3:contains('People') + div a:contains('Raven authentication')" element
  ...
  Scenario: See config page when permitted
    Given the "authenticated" role has the "raven" "administer raven authentication" permission
    And the "authenticated" role has the "system" "access administration pages" permission
  ...
  Scenario: Can't see config page when not permitted
    Given the "authenticated" role does not have the "raven" "administer raven authentication" permission
    And the "authenticated" role has the "system" "access administration pages" permission
    And there is a user called "test0001" with the email address "test0001@example.com"
    When I log in to Raven as "test0001"
    And I go to "/admin/config"
